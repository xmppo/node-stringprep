...
@rem Check for ICU location
if not defined ICU_ROOT goto ICU-not-found
if not exist "%ICU_ROOT%\include\unicode\pwin32.h" goto ICU-not-found
COLOR 6
echo NOTE: Make sure that "%ICU_ROOT%\BIN\" is in your path otherwise you won't be able to load the module!
echo NOTE: You should edit "%ICU_ROOT%\include\unicode\pwin32.h" file in order to replace the "#define U_SIZEOF_WCHAR_T 2" statement to "#define U_SIZEOF_WCHAR_T 1"
COLOR
...
goto exit
:ICU-not-found
echo ICU root path not found! Please check the ICU_ROOT environment variable exists and that it points to the installation folder or ICU 
goto exit
:exit-error
